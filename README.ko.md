...
Adapter 회귀 테스트는 Anthropic, Gemini, OpenAI-compatible API, Ollama, LM Studio에 대해 fake HTTP 성공 fixture를 사용합니다. 테스트 중 실제 네트워크를 열지 않으면서도 공통 `LLMClient` 경로가 provider 요청을 만들고 성공 응답을 파싱하며 raw provider payload를 저장하지 않는지 확인합니다.
