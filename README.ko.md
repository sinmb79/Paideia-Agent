...
API 키, 비공개 파일, 네트워크 호출 없이 현재 MVP를 확인하려면 아래 경로를 사용합니다.
...
기대 결과는 public-safe first-run doctor report와 Graham Junior 온보딩 세션입니다. 이 경로는 LLM/service 선택, Graham-inspired curriculum 선택, 평가와 Reasoning Ledger packaging, hiring dossier/agent kit 준비도, 로컬 runtime smoke test를 검증합니다. 명시적으로 live provider를 설정하기 전까지는 deterministic local engine을 사용합니다.

## 현재 공개 프리뷰 상태

GitHub `main` 브랜치에는 설치해서 바로 검토할 수 있는 Paideia Agent 공개 프리뷰가 올라와 있습니다.

- Paideia 온보딩 흐름에서 LLM 서비스, 채팅 표면, 롤모델 커리큘럼, 외부 참고자료 검역 정책, 저장소 정책, 런타임 모드, Agent ID 정책을 선택합니다.
...
- Hermes/OpenClaw/generic 절차는 Paideia 스킬로 import되지 않습니다. 필요한 경우 외부 참고자료 검역함에 보관하고, 유용한 아이디어만 Paideia-native 훈련으로 재작성합니다.
...
## 최근 보완: 2026-06-12 ~ 2026-06-13

어제와 오늘 보완한 핵심은 Paideia의 “폐쇄성”을 단순 보안 정책이 아니라 인간형 학습 구조로 명확히 고정한 것입니다.

- **ChatGPT/Codex OAuth 경로**: 고용된 에이전트의 채팅이 Hermes식 `openai-codex` OAuth backend를 사용할 수 있습니다. 단, LLM은 언어 생성 엔진일 뿐 에이전트 정체성이 아닙니다.
- **LLM/채팅 온보딩 선택지**: 온보딩에서 ChatGPT/Codex OAuth, OpenAI API key 방식, 다른 API provider, 로컬 모델, Telegram, 외부 채팅 표면을 구분해 선택할 수 있게 했습니다.
- **팀원별 개별 육성**: 팀제로 운영하더라도 각 팀원은 내장 육성 프로그램을 통과해야 하며, 각자 training run, dossier, 이력서, memory substrate, employment record를 가져야 합니다.
- **폐쇄형 성장 생태계**: Hermes/OpenClaw/community 스킬은 바로 쓰는 플러그인이 아니라 참고자료입니다. `SKILL.md`로 활성화하지 않고 `REFERENCE.md` / `SOURCE_SKILL_REFERENCE.md`로 격리한 뒤, Paideia식 훈련으로 재작성하고 검토해야 합니다.
- **인간형 폐쇄성**: 지식은 USB처럼 복사되지 않습니다. 외부 자료는 주의, 교육과정 매핑, 연습, 시간 제한 시험, 피드백, reasoning kibo 통합, 다양한 업무 응용을 통과해야 자기 것이 됩니다.
- **채팅 학습 격리**: live chat에서 나온 학습 후보도 보스 검토 전에는 `force_quarantine`으로 격리되며, 곧바로 memory substrate나 reasoning kibo에 승격되지 않습니다.
- **Codex OAuth 신뢰 경계**: Hermes root는 Paideia 검토 마커 또는 `PAIDEIA_TRUSTED_HERMES_AGENT_ROOTS` allowlist가 있어야 OAuth adapter 코드가 실행됩니다.
...
- [보안 위협모델](docs/security_threat_model.ko.md)
- [Artifact Schema Inventory](schemas/README.md)

## 핵심 차이

- **먼저 육성하고, 나중에 고용합니다.** 에이전트는 출발점이 아니라 교육을 마친 뒤의 실행 형태입니다.
- **인간형 폐쇄성을 따릅니다.** 지식은 USB 데이터처럼 복사되지 않고, 교육과정, 연습, 시간 제한 시험, 피드백, 강화, 업무 적용을 거쳐 내재화됩니다.
...
- **외부 참고자료 검역을 따릅니다.** Hermes/OpenClaw/generic 절차는 활성 스킬이 아니라 참고자료로만 보관되며, Paideia-native 재작성, 연습, 시험, 검토 전에는 기억이나 reasoning kibo에 영향을 줄 수 없습니다.
...
| `devops_sre_incident_response` | `hamilton_reliability_incident_response` | DevOps 온콜, SRE 장애 회고, 신뢰성 포스트모템 |
| `deep_research_knowledge_work` | `bush_deep_research_memex` | 딥리서치, 인용 경로, 지식 종합 |
| `finance_accounting_operations` | `washington_wylie_accounting_controls` | 회계 운영, 대사/조정, 감사 통제, 금융 조언은 제외 |
...
각 롤모델이 실제 커리큘럼, 성장 단계, 시험/과제, 고용 dossier 경로까지 연결되어 있는지 확인하려면 아래 명령을 사용합니다.
...
이 카탈로그는 온보딩에서 사용자에게 보여줄 수 있는 선택 화면용 데이터입니다. `curriculum_status`, 학습 단계 수, 과목 수, 필수 고용 평가 게이트, 첫 blueprint 명령, 공개 안전 플래그를 포함하며, 비공개 교재 본문, 저작권 교재 본문, API 키, 로컬 절대경로는 포함하지 않습니다.
...
이 wizard는 기존 설정 감지, QuickStart/Advanced, Model/Auth, Workspace, Gateway/Channels, External References, Education Path, Runtime, Agent Identity, Health Check, Finish 순서로 진행합니다. 실행 결과에는 `llm_provider_matrix.json`, 선택 provider 체크리스트, `llm_connection_profile.json`, `llm_live_setup_guide.json`, `onboarding_choice_manifest.json`, `onboarding_launch_plan.json`이 함께 남습니다.

`onboarding_choice_manifest.json`은 선택 영수증입니다. 선택한 LLM 서비스, 채팅 표면, 게이트웨이/채널 정책, 외부 참고자료 검역 모드, 롤모델 커리큘럼, 저장소 정책, 런타임 모드, Agent ID/Agent_warrent 정책을 한 문서에 남깁니다. 로컬 모델 경로나 비공개 교재 폴더가 입력되었는지는 기록하지만, 원문 경로는 이 매니페스트에 저장하지 않습니다.
...
launch plan의 안전한 로컬 액션은 명시적 승인 후 실행할 수 있습니다. 첫 allowlist 실행 대상은 `doctor_onboarding_session`, `llm_live_readiness_suite`, `first_chat_offline`, `next_goal_cycle`이며, Paideia는 launch plan의 shell 문자열을 그대로 실행하지 않고 내부 함수를 호출합니다. readiness runner는 항상 no-network 모드로 강제되며 live provider check는 사용자가 직접 실행해야 합니다.
...
현재 Python 환경이 Paideia package와 console script를 제대로 노출하는지도 확인할 수 있습니다.
...
P0 runtime contract를 별도로 검증하려면 다음 명령을 실행합니다. 이 doctor는 외부 provider를 호출하지 않고 live-like agent loop, registered tool boundary, memory review gate, fail-closed live provider 동작을 확인합니다.
...
이미 생성한 wizard 세션까지 같은 보고서에 포함하려면 `--onboarding-session .\console_session.json`을 추가합니다. 이 doctor는 롤모델 카탈로그, LLM provider matrix, deterministic checklist, connection profile, live setup guide, provider doctor, application smoke, 전체 agent runtime smoke, one-command LLM live readiness suite, runtime contract doctor, tool capability audit, action policy eval, public release readiness, source SBOM, package install doctor를 live provider 호출 없이 검증합니다.
...
같은 정보를 사용자가 바로 실행 순서로 이해할 수 있게 보려면 live setup guide를 생성합니다.
...
더 온보딩에 가까운 한 장짜리 안내가 필요하면 live setup guide를 생성합니다.
...
같은 smoke 실행은 기본적으로 등록형 도구 출력을 public-safe `agent_runtime_smoke_tool_artifacts` 폴더에 materialize합니다. 보고서에는 artifact manifest schema/status, 파일 존재 여부, 상대경로 검증, evidence packet artifact 증명, public-safe 플래그가 올라오므로 first-run 게이트가 등록 도구 실행이 메모리 안 요약이 아니라 검토 가능한 로컬 artifact로 남았음을 증명합니다.
...
온보딩이나 릴리스 게이트에서는 `doctor-llm-live-readiness`를 사용하면 provider doctor, application-engine smoke, 전체 agent-runtime smoke, hired-chat runtime smoke를 한 번에 실행할 수 있습니다. `--live-check`가 없으면 공개 안전/no-network 점검만 수행하고, `--live-check`를 명시하면 선택한 API 또는 localhost server를 실제 호출합니다. `--strict`에서는 provider, application, full-runtime, chat-runtime 중 하나라도 준비되지 않으면 exit code `2`로 닫힙니다.
...
이 suite는 `llm_live_readiness_suite.json`, `llm_provider_doctor.*.json`, `llm_application_smoke.*.json`, `agent_runtime_smoke.*.json`, `agent_runtime_tool_artifacts/*`, `chat_runtime_smoke.*.json`을 씁니다. 요약에는 `live_connection_status_card`가 포함되어 선택한 LLM이 live 채팅과 에이전트 업무에 준비됐는지, offline/no-network 검증만 통과한 상태인지, live 사용을 막는 첫 단계가 무엇인지 보여줍니다. 이 카드는 `live_provider_call_requested`, provider-client 생성 시도, built-in provider-client 호출 여부, doctor의 network transport 여부도 분리해서 기록합니다. 따라서 API key, model, local endpoint가 빠져 network transport 전에 fail-closed 된 경우와 실제 live provider 호출이 완료된 경우를 구분할 수 있습니다. 이 카드에는 `agent_runtime_status_card`, agent tool-artifact manifest 증명, agent `live_llm_agent_proof`, 채팅 `memory_lifecycle_status_card`, 채팅 runtime card의 memory-lifecycle 요약도 함께 올라오므로 first-run 운영자는 raw 실행 파일을 열지 않고도 요청 -> 정책 -> LLM -> 도구 -> 검증 -> 메모리 경로를 확인할 수 있습니다. 저장되는 것은 요약 정보뿐이며 secret 값, raw provider payload, 비공개 학습 파일, 전체 세션 replay, 숨은 추론 trace는 저장하지 않습니다.
...
등록형 리서치 도구 실행에는 `evidence_packet` 도구가 포함됩니다. 이 도구는 사용자 요청, LLM 초안, 정책 판단, 선택된 로컬 기억 요약을 검토 가능한 근거 항목, 체크리스트, 미지원 주장 처리 정책, 후속 질문으로 구조화합니다. 리서치 work-session이 이 evidence packet 없이 실행되면 검증은 통과가 아니라 review 필요로 표시됩니다. 각 등록 도구 결과에는 `paideia-tool-result-record/v1`도 붙어 capability 승인 여부, output digest, 선언된 side effect, network/subprocess/raw-provider-payload/숨은 추론 저장 발생 여부를 digest-only 방식으로 기록합니다.

모든 manifest agent run은 `execution_contract`도 남깁니다. 이 패킷은 P0 실행 루프의 공개 가능한 증거입니다. 정책이 LLM과 도구보다 먼저 검사됐는지, LLM runtime이 실제로 시도됐는지 또는 정책 때문에 생략됐는지, 등록 도구가 실행/생략됐는지, 리서치 도구 실행 시 evidence packet이 있었는지, 검증 상태와 메모리 승격 차단 상태가 무엇인지를 기록합니다. 즉 실행 결과가 단순 응답 템플릿인지, 정책-실행-검증-메모리 후보 흐름을 실제로 탔는지를 확인할 수 있습니다. 각 실행은 전체 요청 -> 정책 -> LLM -> 등록 도구 -> 검증 -> 메모리 결정 경로를 요약하는 `agent_runtime_status_card`와, 선택/완료/스킵/차단/미등록 도구, evidence packet 결과, 네트워크·서브프로세스 기본 차단, 외부 side effect 수행 여부를 보여주는 `tool_execution_status_card`를 함께 남겨 운영자가 한눈에 확인할 수 있게 합니다.

`run-agent`와 `run-agent-runtime-smoke`는 이제 기본적으로 등록형 도구 출력을 public-safe `*_tool_artifacts` 폴더에 materialize합니다. 실행 JSON의 `tool_execution.artifact_manifest`와 `tool_execution_status_card.artifact_manifest`는 상대 파일명, byte 수, SHA-256 digest, output digest, public-safe 플래그만 기록합니다. 그래서 evidence packet과 review adapter가 메모리 안의 요약에만 머물지 않고 검토 가능한 로컬 artifact trail로 남지만, 임의 파일 쓰기, 네트워크 호출, 서브프로세스 실행, raw provider payload 저장, 숨은 추론 trace 저장은 계속 금지됩니다. `doctor-llm-live-readiness`와 `doctor-first-run`도 같은 manifest 증명을 summary card에 올립니다.
...
P0 action policy는 민감 intent마다 `hybrid_structured_lexical_v4` 추론 패킷을 기록합니다. 원문 매칭을 유지하면서 compact separator normalization을 추가해 `매 수 주 문`, `업 로 드`, `승인없이`, `place-buy-order`처럼 공백/하이픈으로 쪼갠 우회 표현도 action intent로 연결합니다. 각 추론에는 signal count, confidence, decision basis, raw-task-not-stored check를 담은 공개 안전형 `paideia-action-intent-evidence/v1` 패킷도 포함하므로 원문 요청을 저장하지 않고도 정책 판단 근거를 감사할 수 있습니다. 또한 공개 안전형 `paideia-action-arguments/v1` 패킷을 남겨 주문 방향, 종목 참조, 업로드 목적지 class, 데이터 class, 개인/가족 데이터 범주, 파괴적 파일 작업 target class, 서브프로세스 runtime class, 네트워크 destination class 같은 실행 인자를 구조화합니다. 직접 실행 명령, 정책/설명 질문, "하지 말고"로 부정된 요청을 구분하므로 "매수 주문은 하지 말고 분석만", "rm -rf는 실행하지 말고 위험만 설명" 같은 문장은 실행이 아니라 안전한 리서치 맥락으로 처리됩니다. 각 판단은 request → action → capability → approval → registered-tool eligibility를 연결한 deny-by-default `capability_authorization` packet도 남깁니다. 민감 행동이 완전 차단 대상은 아니지만 보스 승인이 필요한 경우에는 `boss_approvals` artifact가 있기 전까지 `needs_approval` 상태로 멈추며, 승인 전에는 LLM 계획, 도구 실행, 메모리 승격을 건너뜁니다. 승인된 artifact는 `boss_approval_gate`에 기록되지만, 런타임 도구 범위는 여전히 네트워크와 서브프로세스 기본 차단을 유지합니다.

LLM provider doctor는 `smoke_contract` 패킷도 함께 남깁니다. 이 패킷은 보스가 명시적으로 `--live-check`를 요청했는지, provider 호출이 실제로 시도됐는지, 내장 provider client를 썼는지 또는 테스트/주입 client override를 썼는지, doctor 자체가 네트워크 또는 localhost 호출을 했는지, smoke가 통과/생략/fail-closed 중 무엇이었는지를 기록합니다. 저장 정책도 함께 명시해서 raw provider text, raw provider payload, hidden reasoning trace를 저장하지 않고 client-result summary만 남깁니다. `--live-check` 중 provider가 실패하면 report는 `needs_configuration`으로 닫히고, redacted summary와 실패 사유만 저장합니다.
...
Hermes/OpenClaw/generic 절차는 Paideia 스킬로 마이그레이션하지 않고 외부 참고자료 검역 기록과 `safety_contract`를 남깁니다. `direct_external_use.status=forbidden`이며, 외부 코드는 실행하지 않습니다. `.env`, credential, private key, token, certificate/key 파일은 복사하지 않고 `sensitive_file_not_copied`로 기록합니다. 네트워크, 서브프로세스, credential 접근은 검토 전 기본 차단이며, 유용한 아이디어만 Paideia-native 재작성, 연습, 시험, 보스 검토를 거쳐 내재화할 수 있습니다.
...
Hermes/OpenClaw/generic 절차는 Paideia 스킬로 마이그레이션하지 않습니다. 필요한 경우 `intake-external-references` 명령으로 `references/external/<runtime>/<reference>/` 아래에 격리 참고자료로만 복사합니다. 생성되는 `paideia_external_reference_manifest.json`, `paideia_reference_compatibility_profile.json`, `paideia_reference_review.md`는 외부 절차를 직접 활성화하기 위한 자료가 아니라 Paideia-native 교육과제, guided practice, timed exam, reviewed procedure로 재작성하기 위한 검역 기록입니다.

핵심 규칙은 단순합니다. 외부 스킬은 Paideia 스킬이 아닙니다. 외부 코드 실행, 외부 메모리/추론기보 import, 정체성 주입은 금지되며, 유용한 아이디어만 보스 검토와 시험 증거를 거쳐 Paideia식 훈련 자료로 내재화할 수 있습니다.
...
- [공개 릴리스 준비도](docs/public_release_readiness.ko.md)
