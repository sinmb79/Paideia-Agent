...
- 천재 도출 엔진을 추가했습니다. 각 인재는 모델 크기나 컴퓨팅 파워가 아니라 고정된 용량 안에서 전공 주의 배분, 패턴 chunking, 시간 제한 시험, 명시적 약점 가드레일, 검토된 전이 업무를 통해 특정 분야의 좁은 천재성을 훈련합니다.
...
- **천재 도출 엔진**: Paideia는 천재성을 일반 초지능이나 raw compute scaling으로 보지 않습니다. `*_genius_profile.json`을 생성해 특정 전공의 압축 문제해결 훈련, 패턴 chunk 후보, 약점 가드레일을 dossier, memory substrate, release bundle, installed agent, graduate package에 연결합니다.
- **증거 기반 천재성 검증**: 블루프린트만으로 만든 천재 프로필은 `needs_training_evidence` 초안입니다. 평가 통과, 검토된 과제/업무, 성장 프로필 또는 학년별 학습 기록이 있어야 `passed`가 됩니다.
...
- **Genius Derivation Profile**은 커리큘럼, 시험, 성장 비대칭, 패턴 chunk, 약점 가드레일을 특정 분야의 검증 가능한 전문 천재성 훈련 경로로 압축합니다.
...
- **안전한 스킬 마이그레이션을 따릅니다.** Hermes/OpenClaw/generic 스킬은 가져올 수 있지만, 재작성, 테스트, 검토 전에는 참고자료로만 남습니다.
...
- `genius_profile.json`: 특정 전공의 패턴 chunk, 시간 제한 훈련, 약점 가드레일, 검증 상태를 담은 천재 도출 프로필
- `hiring_dossier.json`
...
세부 설명은 [Growth Profile v0.5-v0.7](docs/growth_profile_v05.ko.md)와 [Paideia 천재 도출 엔진](docs/genius_derivation_engine.ko.md)을 참고하세요.
