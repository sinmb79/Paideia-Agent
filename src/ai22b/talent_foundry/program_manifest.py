...
def _commands() -> list[dict[str, str]]:
    purposes = {
        ...
        "onboard": "Run the Paideia first-run wizard for model, workspace, channel, external references, education path, identity, and health setup.",
        "show-onboarding-dashboard": "Read an onboarding launch plan and render its OpenClaw-style dashboard cards and next-action queue without executing commands.",
        ...
        "build-llm-connection-profile": "Build a no-network setup and verification profile for one selected LLM provider.",
        "build-llm-live-setup-guide": "Build a no-network owner-facing guide for model, key, localhost, live-check, and chat readiness steps.",
        "show-llm-connection-status": "Render a one-screen no-network LLM connection status card with setup blockers, next actions, and safe live-check commands.",
        ...
        "audit-tool-capabilities": "Audit the registered local tool registry, capability grants, and deny-by-default execution contract.",
        "create-boss-approval": "Create a local boss approval artifact for one sensitive action gate without executing that action.",
        "run-llm-application-smoke": "Run a selected LLM through the Paideia application-engine path and write a public-safe runtime report.",
        "run-agent-runtime-smoke": "Run a selected LLM through the full Paideia agent loop and write a public-safe runtime report.",
        "run-chat-runtime-smoke": "Run a selected LLM and chat surface through the hired-chat path and write a public-safe report.",
        "doctor-llm-live-readiness": "Run provider doctor, application smoke, agent runtime smoke, and chat smoke as one provider readiness suite.",
        ...
        "doctor-llm-adapters": "Verify public-safe LLM adapter contracts without live provider or localhost calls.",
        ...
        "export-agent-warrent-registration-request": "Build the Agent_warrent POST /agents/register body draft for explicit owner signing and manual submission.",
        "build-agent-warrent-connector-kit": "Build an owner-controlled Agent_warrent SDK bridge kit from a local registration request without signing, uploading, or calling the network.",
        ...
        "doctor-paideia-kit-first-run": "Doctor an install kit and run the first offline chat smoke before live provider use.",
        "intake-external-references": "Copy Hermes/OpenClaw/generic procedures into external-reference quarantine without activating them as Paideia skills.",
