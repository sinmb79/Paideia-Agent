...
The WebChat page also exposes `/api/runtime` and `/api/smoke-plan`. Those endpoints show the selected OpenClaw provider/model, chat surface, channel route, live runtime path, and live smoke-test sequence without reading or storing provider keys, bot tokens, OAuth refresh tokens, QR sessions, or private training files. Browser messages can choose `offline`, `auto`, or `live` per turn, and the response records the requested runtime mode/model override.
