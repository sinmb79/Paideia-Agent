...
CI uses read-only repository token permissions, `actions/checkout@v6` with `persist-credentials: false`, Node 24-capable official GitHub Actions, and Dependabot monitoring for GitHub Actions. Security and release-gate jobs upload their JSON reports as 14-day workflow artifacts so reviewers can inspect Bandit, pip-audit, doctor, SBOM, and public hygiene evidence from the run. The release-readiness audit parses workflows with PyYAML for job, matrix, action, credential, and artifact-retention policy checks. A separate scheduled optional-dependency audit checks `live-llm`, `local-llm`, `rag`, `fine-tune`, and `all` extras without slowing every pull request.
