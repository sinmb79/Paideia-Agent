...
- 다음 단계: 보스가 외부 등록을 명시적으로 수행한 뒤 반환되는 `ail_id`, credential token, 서명 검증 상태를 dossier와 install manifest에 재연결하는 import 흐름을 추가합니다.
