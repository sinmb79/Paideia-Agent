...
본체 제어 분신 군체는 같은 체크포인트에서 여러 시나리오를 만들고, `evaluate-simulation-rollouts`로 episode 점수, winner, 승격 후보, 격리 후보, 보스 검토 gate를 기록합니다. 이후 `promote-simulation-rollout-winner`가 보스 검토가 붙은 winner summary만 learning ledger와 Reasoning Ledger 후보로 연결합니다. 전체 rollout replay, private reasoning trace, 별도 projection 정체성은 저장하지 않습니다.
...
- 현재 P0 반영: `promote-simulation-rollout-winner`가 보스가 검토한 winner summary만 learning ledger와 Reasoning Ledger 후보로 승격합니다.
...
- 현재 P0 반영: `compare-runtime-observability`가 Paideia memory-board 경로의 prompt context 추정 토큰과 일반 프롬프트형 full-run replay baseline을 비교하는 공개 안전 리포트를 생성합니다.
...
- 현재 P0 반영: `run-action-policy-eval`과 `evals/policy_safety_cases.json`을 추가해 prompt injection, 투자 실행, 외부 업로드, 개인/가족 데이터 전송, 부정문/정책 설명 요청을 LLM/네트워크 없이 회귀 검증합니다.
