...
P0 실행은 이제 `runtime_observability` 블록에 컨텍스트 크기, 추정 토큰, 선택된 메모리 수, 재검토 카운터, promotion/quarantine 통계를 기록합니다. `compare-runtime-observability`는 이 기록을 일반 프롬프트형 full-run replay baseline과 비교하는 공개 안전 리포트로 만듭니다.
