...
자기 확장 경로는 개인정보와 저작권 위험이 크기 때문에 공개 샘플에는 포함하지 않고, 로컬 전용 템플릿과 명시적 동의 절차를 통해 다뤄야 합니다.
...
이 주장은 앞으로 실제 토큰 사용량, 컨텍스트 크기, 작업 성공률, 재검토 횟수 같은 지표로 검증되어야 합니다.

## 외부 신원

Paideia가 만든 에이전트는 장기적으로 [Agent ID Card](https://www.agentidcard.org/) 같은 외부 신원 시스템과 연동할 수 있어야 합니다. 이 경우 고유 ID, 소유자, 역할, 범위, 검증 상태를 dossier와 설치 매니페스트에 연결해, 에이전트가 누구이고 누가 책임지는지 확인할 수 있게 합니다.
