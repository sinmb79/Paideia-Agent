...
| --- | --- | --- |
| OpenClaw onboarding wizard | First-run setup should choose provider/model, gateway mode, chat/channel path, external-reference posture, and next steps. | `start-console` now asks for LLM service and chat surface before the owner request. The generated onboarding session records the selected service, selected surface, catalogs, provider checklist, connection profile, and first-run flow. |
...
| Hermes Agent repository | Modern agents expose setup, model switching, CLI chat, tools, skills, memory, gateways, and migration. | Paideia keeps useful runtime ideas but places external procedures in reference quarantine instead of migrating them into talent identity; install kits, doctor checks, adapter manifests, and the `Reasoning Ledger` remain local growth records. |
