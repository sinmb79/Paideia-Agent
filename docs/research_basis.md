...
| OpenClaw full channel directory | OpenClaw supports chat channels across mainstream messaging, developer/self-hosted systems, regional platforms, WebChat, and voice call plugins; current docs recommend BlueBubbles for new iMessage setups and keep legacy iMessage separate. | `list-openclaw-channel-connectors` and `doctor-openclaw-channel-connectors` now cover every channel in the OpenClaw manifest. The doctor distinguishes direct Paideia adapters from normalized-gateway-only channels and records plugin/bridge requirements such as BlueBubbles macOS server, WhatsApp QR pairing, signal-cli, Matrix credentials, legacy iMessage bridge, Twilio, or regional platform tokens. |
