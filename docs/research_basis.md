...
| OpenClaw full channel directory | OpenClaw supports 26 chat channels across mainstream messaging, developer/self-hosted systems, regional platforms, WebChat, SMS, and voice call plugins. | `list-openclaw-channel-connectors` and `doctor-openclaw-channel-connectors` now cover every channel in the OpenClaw manifest. The doctor distinguishes direct Paideia adapters from normalized-gateway-only channels and records plugin/bridge requirements such as WhatsApp QR pairing, signal-cli, Matrix credentials, iMessage bridge, Twilio, or regional platform tokens. |
