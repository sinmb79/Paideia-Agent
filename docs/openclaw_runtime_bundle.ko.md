...
- OpenClaw Gateway의 OpenAI 호환 HTTP surface(`/v1/chat/completions`)를 쓸 수 있도록 `gateway.http.endpoints.chatCompletions.enabled=true` patch를 포함합니다.
...
- `openclaw_config_patch.json`: OpenClaw식 agent, `models.providers`, channel, `channels.modelByChannel`, `bindings[]`, OpenAI 호환 Gateway HTTP endpoint 설정 patch입니다.
