...
first-run smoke suite는 `paideia-llm-provider-matrix/v1`, deterministic local engine 기반 `paideia-llm-onboarding-checklist/v1`, `paideia-llm-connection-profile/v1`도 생성합니다. 이를 통해 LLM/service 선택 경로가 모든 선택 가능한 provider를 나열하고, setup requirement, provider doctor, live-check, application smoke, agent runtime smoke, 첫 채팅 명령을 기본 provider 호출이나 secret 유출 없이 만들 수 있음을 증명합니다.
...
agent runtime smoke 보고서에는 `paideia-live-llm-agent-proof/v1` 공개 안전 proof packet이 포함됩니다. 이 packet은 선택된 경로를 offline 검증, 주입된 live-like client, built-in live provider client, adapter context, fail-closed 설정 게이트로 구분합니다. 따라서 release review는 raw provider payload나 숨은 추론 trace를 저장하지 않고도 no-network CI 증거와 실제 live provider client 실행을 구분할 수 있습니다.
