...
| Imported skill | 안전하지 않은 코드나 과도한 권한을 포함할 수 있습니다. | 검토 전까지 격리 및 비활성 상태를 유지합니다. |
...
- 악성 Hermes/OpenClaw/generic imported skill.
...
flowchart LR
    ...
    ImportedSkill["Imported skill package"] --> Quarantine["격리 profile"]
...
| imported skill -> runtime | disabled wrapper와 capability manifest. | 보스 검토 전 직접 실행. |
...
1. 영향을 받은 generated kit 또는 imported skill 사용을 중지합니다.
