...
MVP는 비공개 파일, API 키, 생성 체크포인트, 네트워크 호출 없이 실행되어야 합니다. live LLM, external reference quarantine, Agent_warrent 등록, projection swarm은 deterministic local 경로가 검증된 뒤 연결합니다.
...
| External reference quarantine | Experimental | Hermes/OpenClaw/community 절차는 참고자료로만 격리되며, Paideia-native 재작성, 연습, 시험, 검토 없이는 활성 Paideia 스킬이 될 수 없습니다. |
...
3. prompt injection, memory poisoning, external reference source, provider secret, generated kit에 대한 security threat model을 유지합니다.
